...
public class QAInterator implements MultiDataSetIterator {
    ...
    public QAInterator(int seed, int batchSize, int totalBatches,
                       int vocabSize,
                       Stream<QAState> qaStates, int timestep) {
        ...
        this.encoderSeqLength = qaStates.flatMap(p -> p.getQuestions().stream())
                .mapToInt(String::length).max().orElse(10);
        this.decoderSeqLength = qaStates.flatMap(p -> p.getAnswers().stream())
                .mapToInt(String::length).max().orElse(10) + 1;
        this.outputSeqLength = this.decoderSeqLength; // (numdigits + 1)max the sum can be and "."
    ...
    public MultiDataSet next(int sampleSize) {
        ...
        INDArray encoderSeq = Nd4j.zeros(sampleSize, vocabSize, encoderSeqLength);
        INDArray decoderSeq = Nd4j.zeros(sampleSize, vocabSize, decoderSeqLength);
        INDArray outputSeq = Nd4j.zeros(sampleSize, vocabSize, outputSeqLength);
